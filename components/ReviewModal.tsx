...
export default function ReviewModal() {
  ...
  const {
    ...
    setReviewModal,
    reviewModal
  } = useContext(ReviewModalContext) as ReviewModalProviderType
  ...
  const goToPage = (id: string) => {
    closeModal()
    if (type === "album") {
      router.push(`/new-review/album/${id}`)
    } else {
      router.push(`/new-review/track/${id}`)
    }
  }
        ...
        <div ref={modalRef} className="flex flex-col space-y-8 p-10 w-full md:w-3/5 lg:w-2/5 rounded-md border border-gray-700 bg-gray-950">
          ...
          <div className="flex flex-col space-y-5">
            <div className="relative w-full text-xs md:text-sm">
              ...
              {
                dropdown && 
                <div ref={dropdownRef} className="z-50 absolute flex flex-col top-14 border rounded-md bg-gray-950 border-gray-700 w-full">
                  <button
